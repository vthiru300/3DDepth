...
from mydetector3d.utils import box_utils, common_utils
...
class WaymoDataset(DatasetTemplate):
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None):
        ...
        self.sample_sequence_list = [x.strip() for x in open(split_dir).readlines()] #tfrecord file name

        self.infos = []
        self.seq_name_to_infos = self.include_waymo_data(self.mode) #retur none, data saved in infos
        ...
        if self.dataset_cfg.get('USE_PREDBOX', False): #No
    ...
    def include_waymo_data(self, mode):
        ...
        num_skipped_infos = 0
        for k in range(len(self.sample_sequence_list)): #50 tfrecord files
            sequence_name = os.path.splitext(self.sample_sequence_list[k])[0] #remove .tfrecord
            info_path = self.data_path / sequence_name / ('%s.pkl' % sequence_name) #pkl file in processed folder
            info_path = self.check_sequence_name_with_all_version(info_path)
            if not info_path.exists():
                num_skipped_infos += 1
                continue
            with open(info_path, 'rb') as f:
                infos = pickle.load(f) #size 50 dict array 200frame/downsamplerate(4)=50, each dict
                waymo_infos.extend(infos)

            seq_name_to_infos[infos[0]['point_cloud']['lidar_sequence']] = infos #save to one dict with sequence as the name

        self.infos.extend(waymo_infos[:]) #waymo_info contains 2492 dicts
        self.logger.info('Total skipped info %s' % num_skipped_infos)
        self.logger.info('Total samples for Waymo dataset: %d' % (len(waymo_infos))) #2492

        if self.dataset_cfg.SAMPLED_INTERVAL[mode] > 1: #No
            sampled_waymo_infos = []
    ...
    def get_infos(self, raw_data_path, save_path, num_workers=multiprocessing.cpu_count(), has_label=True, sampled_interval=1, update_info_only=False):
        ...
        from mydetector3d.datasets.waymo import waymo_utils
    ...
    def get_lidar(self, sequence_name, sample_idx):
        lidar_file = self.data_path / sequence_name / ('%04d.npy' % sample_idx) #lidar file name based on frame index
        point_features = np.load(lidar_file)  # (N,6) (N, 6): [x, y, z, intensity, elongation, NLZ_flag (most==-1)]
        #Each scene may include an area that is not labeled, which is called a “No Label Zone” (NLZ)
        points_all, NLZ_flag = point_features[:, 0:5], point_features[:, 5]
        if not self.dataset_cfg.get('DISABLE_NLZ_FLAG_ON_POINTS', False): #not used
            points_all = points_all[NLZ_flag == -1]
        points_all[:, 3] = np.tanh(points_all[:, 3]) #limit the intensity from -1 to 1
    ...
    def __getitem__(self, index):
        ...
        if 'annos' in info:
            ...
            if self.dataset_cfg.get('INFO_WITH_FAKELIDAR', False):
                ...
                gt_boxes_lidar = annos['gt_boxes_lidar']#[54,9]
...
def create_waymo_infos(dataset_cfg, class_names, data_path, save_path,
                       ...
                       workers=min(4, multiprocessing.cpu_count()), update_info_only=False):
...
import pandas as pd
#OpenPCdet version, directory should contain the original waymo tfrecord file
def create_trainvaltestsplitImageSets(dataset_dir, output_dir):
    trainingdir = Path(dataset_dir) #os.path.join(dataset_dir, 'image_0')
    ...
    if not os.path.exists(ImageSetdir):
        ...
        dataset = []
        for img in images: #tfrecordfile
            #new add
            head, tail = os.path.split(img) #"tail" is everything after the final slash.
            fileidname=tail # tail[:-4]
            print('fileidname',fileidname)
            dataset.append(fileidname)
        ...
        writepdrow_to_file(os.path.join(ImageSetdir, 'trainval.txt'), df['index'])
        writepdrow_to_file(os.path.join(ImageSetdir, 'train.txt'), X_train['index'])
        writepdrow_to_file(os.path.join(ImageSetdir, 'val.txt'), X_val['index'])
    else:
        images = os.listdir(trainingdir)
        print('ImageSet already exist:', len(images))

def create_trainvaltestsplitImageSets2(filelist, output_dir):
    ImageSetdir = os.path.join(output_dir, 'ImageSets')
    if not os.path.exists(ImageSetdir):
        os.makedirs(ImageSetdir)
        # totalimages=len([img for img in images])
        dataset = []
        for img in filelist: #tfrecordfile
            #new add
            head, tail = os.path.split(img) #"tail" is everything after the final slash.
            fileidname=tail # tail[:-4]
            print('fileidname',fileidname)
        ...
        df = pd.DataFrame(dataset, columns=['index'])
        X_train, X_val = train_test_split(df, train_size=0.8, test_size=0.2, random_state=42)
        print("Train size:", X_train.shape) #248
        print("Val size:", X_val.shape) #198
        writepdrow_to_file(os.path.join(ImageSetdir, 'trainval.txt'), df['index'])
        writepdrow_to_file(os.path.join(ImageSetdir, 'train.txt'), X_train['index'])
        writepdrow_to_file(os.path.join(ImageSetdir, 'val.txt'), X_val['index'])
    else:
        #images = os.listdir(trainingdir)
        print('ImageSet already exist:', len(filelist))

def writepdrow_to_file(path, data): 
    file = open(path, 'w') 
    for row in data:
...
def myget_infos(sequence_list, save_path, processed_data_tag='mytest', split='train', num_workers=8, has_label=True, sampled_interval=4, update_info_only=False):
        #from . import waymo_utils
        from mydetector3d.datasets.waymo import waymo_utils
        print('---------------The waymo sample interval is %d, total sequecnes is %d-----------------'
              % (sampled_interval, len(sequence_list)))
        
        sequence_infos = []
        savepath = os.path.join(save_path, processed_data_tag)
        for sequencefile in sequence_list:
            info = waymo_utils.process_single_sequence(Path(sequencefile), save_path=Path(savepath), sampled_interval=sampled_interval, has_label=has_label, update_info_only=update_info_only)
        ...
        # )
        # sample_sequence_file_list = [
        #     raw_data_path / sequence_file for sequence_file in sequence_list
        # ]
        ...
        train_filename = save_path / ('%s_infos_%s.pkl' % (processed_data_tag, split))
        with open(train_filename, 'wb') as f:
...
if __name__ == '__main__':
    ...
    if args.func == 'create_waymo_infos':
        ...
        create_waymo_infos(
            ...
            update_info_only=args.update_info_only #False
        )
